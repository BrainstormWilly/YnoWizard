...
import com.ynotasting.finder.model.service.fb.FacebookService;
...
public class WineReviewActivity extends SherlockFragmentActivity implements IFacebookContext, IActionBarActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if( _fbSvc.isSessionValid() ){
			...
			title.setText("Log into Facebook?");
			subtitle.setText("Wine reviews can only be stored to your Facebook timeline.");
	...
	public void onDialogComplete(Bundle $data, String $service) {
		if( $service.equals("authorize") ){
			...
			Toast.makeText(getApplicationContext(), "Review posted to your feed.", Toast.LENGTH_SHORT).show();
		}
	}
			...
			_fbHdl.post(new Runnable() {
                ...
                public void run() { 
                	try{
                		...
                		Toast.makeText(getApplicationContext(), "Review posted to your Yno Wizard activities", Toast.LENGTH_SHORT).show();
                		if( _sendToFeed )
                			_fbSvc.publishReviewToFeed(_review);
                	}catch(JSONException $e){
                		String msg = "Unable to post review. You may need to re-log into your Facebook account.";
                		try{
                			JSONObject respObj = new JSONObject(resp);
	...
	public void onServiceCancel(String $service) {
		if($service.equals("authorize")){
			...
			Toast.makeText(getApplicationContext(), "Feed post cancelled.", Toast.LENGTH_SHORT).show();
		}
	}
